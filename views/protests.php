<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Protests</h1>
    </div>
    <!-- DataTales Example -->
            ...
            <div class="modal-body">
                <form class="forms-sample" id="formEntry" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-input" id="event_category" name="event_category" placeholder="Event Category" required>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function renderData() {
        ...
        table = $("#tblEntry").DataTable({
            ajax: "ajax/get_event_protests.php",
            columns: [{
                    data: 'count'
                },
                {
                ...
                },
                {
                    mRender: function(data, type, row) {
                        if (row.status == "Pending") {
                            return `<button type="button" class="btn btn-primary btn-rounded btn-icon btn-sm" onclick="resolve(${row.protest_id})" ><i class="fas fa-check-circle"></i> Resolve</button>`;
                        } else {
    ...
    function resolve(protest_id) {
        Swal.fire({
            icon: 'question',
            title: 'Protests',
            text: 'Are you sure to resolve entry?',
            showCancelButton: true,
            allowOutsideClick: false
        }).then((result) => {
            /* Read more about isConfirmed, isDenied below */
            if (result.isConfirmed) {
                $.post("ajax/resolve_protest.php", {
                    protest_id: protest_id
                }, function(data, status) {
                    if (data == 1) {
                        success_update("Protests");
                    }
                    renderData();
                });
            } else {}
